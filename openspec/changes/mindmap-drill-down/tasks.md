...
- [ ] 1.1 In `MindMapTree.tsx`, add `const [drillNodeId, setDrillNodeId] = useState<string | null>(null)`
- [ ] 1.2 Extend the `effectiveTree` `useMemo` to support `drillNodeId`:
...
- [ ] 1.3 When `drillNodeId` is set, force the drill node's children to be visible regardless of `collapsedIds`:
...
- [ ] 1.4 When `drillNodeId` is cleared (set to `null`), the existing behaviour is fully restored
...
- [ ] 2.1 Create `src/components/flow-shell/DrillBreadcrumb.tsx`:
...
- [ ] 2.2 Create `src/components/flow-shell/__tests__/DrillBreadcrumb.test.tsx`:
...
- [ ] 2.3 Mount `DrillBreadcrumb` in `MindMapTree.tsx`:
...
- [ ] 3.1 Add `drillDown` to `MENU_ORDER` in `MindMapContextMenu.tsx`, positioned between `'center'` and `'duplicate'`
- [ ] 3.2 Add `hasChildren: boolean` to `ContextMenuProps`
- [ ] 3.3 Add `onDrillDown` to `ContextMenuProps` callback type
- [ ] 3.4 Add `case 'drillDown'` to the menu action switch in `trigger()` → calls `onDrillDown()`
- [ ] 3.5 Add `'drillDown'` to `MenuKey` union type
- [ ] 3.6 Render the menu item:
...
- [ ] 3.7 Wire the callback in `MindMapTree.tsx`:
...
- [ ] 4.1 In `MindMapTree.tsx`, when `drillNodeId` is set, pass `effectiveTree` (the drilled subtree) instead of full `tree` to `matchNodes()` for search:
  - `searchMatchNodeIds` memo reads `effectiveTree` instead of `tree` when drilling
  - The `searchQuery` prop/state remains unchanged
- [ ] 4.2 In `MindMapTree.tsx`, when `drillNodeId` is set, pass `effectiveTree` to `MindMapOutline` instead of `activeMindmap?.tree`:
...
- [ ] 4.3 Verify that exiting drill-down restores full-tree scope for both search and outline
...
- [ ] 5.1 `npm run build` passes (tsc -b + vite build)
- [ ] 5.2 `npm run lint` passes (eslint .)
- [ ] 5.3 `npm test` passes — all existing tests green, new tests added
- [ ] 5.4 Manual smoke test:
  - [ ] 5.4.1 Open mindmap with multi-level tree, right-click a node with children → "下钻到此" visible and functional
  - [ ] 5.4.2 After drilling, canvas shows only the focused subtree, dagre re-layouts with the new root
  - [ ] 5.4.3 Breadcrumb shows correct path, clicking `🏠 全部` exits drill
  - [ ] 5.4.4 Breadcrumb intermediate segment click re-drills at that level
  - [ ] 5.4.5 Right-click a leaf node → "下钻到此" not visible
  - [ ] 5.4.6 Initially collapsed node → after drilling, children are visible (auto-expanded)
  - [ ] 5.4.7 Exit drill → collapsed state restored
  - [ ] 5.4.8 Search in drill mode → only matches in subtree
  - [ ] 5.4.9 Outline in drill mode → only shows subtree nodes
  - [ ] 5.4.10 Refresh page → back to full tree view (no persistence)
