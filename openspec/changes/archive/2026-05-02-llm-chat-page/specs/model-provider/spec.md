...
系统 SHALL 对存储的 API 密钥采取基本保护措施。API 密钥在界面显示时 SHALL 默认遮蔽，仅用户点击显示时可见。API 密钥 SHALL 存储在浏览器 IndexedDB 中，不通过网络发送到任何第三方服务器（仅发送到用户配置的 API 端点）。

#### Scenario: API key display masking
- **WHEN** 用户查看提供商配置
- **THEN** API 密钥字段默认显示为遮蔽状态（如 "sk-************"），用户可点击图标切换显示

#### Scenario: API key not sent to third party
- **WHEN** 用户发送对话消息
- **THEN** API 密钥仅在请求用户配置的 API 端点时作为 Authorization 头发送，不发送到其他任何地址
