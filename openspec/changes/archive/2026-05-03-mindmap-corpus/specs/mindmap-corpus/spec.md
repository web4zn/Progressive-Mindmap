...
#### Scenario: Add text fragment via context menu
- **WHEN** 用户选中 AI 回答中的一段文本，右键选择「加入语料库」
- **THEN** 创建 CorpusEntry，`selectedText` 为选中文本，`range` 记录偏移
...
系统 SHALL 在脑图面板中展示当前图谱的语料库列表。列表 SHALL 按来源对话分组折叠显示。每条语料 SHALL 显示：内容摘要（selectedText 或消息前 60 字符）、启用开关、备注图标、删除按钮。
